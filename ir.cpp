...
//If they are a nonreg then this code will print their ip address after averything they say. We don't want that. 
...
//end of removal 
               ...
               {
                  // mChatFifo[ mChatTail ].mData[ lLen++ ] = '(';
                  lLen += StrMaxCopy( mChatFifo[ mChatTail ].mData+lLen, "" , IR_CHAT_MESSAGE_LEN/4 );
                  // mChatFifo[ mChatTail ].mData[ lLen++ ] = ')';
               }
  ...
  if( !strncmp( pMessage, "/drive", 6 ) )
         {
  IRState* lState = NULL;
mUser[ pUserIndex ].mKeyID.mMajor=0;
mUser[ pUserIndex ].mKeyID.mMinor=10;
 mUser[pUserIndex].mTimeStamp = 0;
 //mUser[ pUserIndex ].mTimeStamp          = mTimeStamp;
        
//PrintStateChange( int pPlayerIndex, int pPlayerId, int pTimeStamp  )
  //         mUser[ pUserIndex ].mKeyID.mMajor=0;
...
//28/6/08 - Johnsie
//code REM'd 
//Got rid of the bit that shows a users ip in a chat        
...
{
   char lUserName[60];
fprintf( stderr, "Adding User lUserName\n" );
//char* lUserName;
...
//change by johnsie
  sprintf( lUserName, "%s%c", pUserName, lAliasIndex );
         }
...
//disabled by John Mcourt
// We dont use the patch anymore
//Re-activate if we want to use a patch again
...
//remmed out by Johnsie so that nonregs can start races
/*
...
mPort = accept( pParent, (struct sockaddr*)&lAddr, (socklen_t*)&lAddrSize ); 
      ...
      #endif
      {
         ...
         #endif
         {
            ...
            else
            {
  			...
  			//	system("php adduser.php?username=lUserName");
